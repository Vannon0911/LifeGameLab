...
const AGENTS_ROOT = new URL("../../llm-entry-sequence/", import.meta.url).pathname;
const SHARED_DIR = join(AGENTS_ROOT, "_shared");

/**
 * Laedt eine Datei, gibt "" zurueck wenn nicht vorhanden.
 */
...
export async function loadRole(rolePath) {
  ...
  const [agentMd, skillMd, baseRules, reportSchema] = await Promise.all([
    safeRead(join(fullPath, "AGENT.md")),
    safeRead(join(fullPath, "SKILL.md")),
    safeRead(join(SHARED_DIR, "BASE_RULES.md")),
    safeRead(join(SHARED_DIR, "REPORT_SCHEMA.md")),
  ]);

  if (!agentMd) {
    throw new Error(`No AGENT.md found at ${fullPath}`);
  }
...
export async function loadAllRoles() {
  const roles = {};
  for (const [key, path] of Object.entries(ROLE_PATHS)) {
    try {
      roles[key] = await loadRole(path);
    } catch (err) {
      console.warn(`Warning: Could not load role '${key}': ${err.message}`);
    }
  }
  return roles;
}
