...
- Die Pflichtkette bleibt fest: `agents/orchestrator` → `classify` → `spawn-proof` → `entry` → `ack` → `check` → `cache-sync` → Commit, und jeder neue Cycle startet mit `cache-validate` (`docs/WORKFLOW.md:20-60`).
