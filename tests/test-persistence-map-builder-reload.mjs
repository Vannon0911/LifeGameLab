...
try {
  ...
  assert.equal(typeof raw, "string", "default web persistence must write a JSON payload");
  ...
  const reloadStore = createStore(manifest, { reducer, simStep: simStepPatch });
...
console.log("PERSISTENCE_MAP_BUILDER_RELOAD_OK mapspec tilePlan survives reload and GEN_WORLD");
