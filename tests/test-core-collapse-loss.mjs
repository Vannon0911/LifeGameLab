import { startEvidenceCase } from "./support/liveTestKit.mjs";
startEvidenceCase("test-core-collapse-loss.mjs");

import { makeInitialState } from "../src/project/project.logic.js";
import { GAME_RESULT, RUN_PHASE, WIN_MODE, ZONE_ROLE } from "../src/game/contracts/ids.js";
import { applyWinConditions } from "../src/game/sim/reducer/winConditions.js";

function assert(cond, msg) {
  if (!cond) throw new Error(msg);
}

const state = makeInitialState();
state.meta.playerLineageId = 1;
state.world = {
  w: 4,
  h: 4,
  alive: new Uint8Array(16),
  lineageId: new Uint32Array(16),
  zoneRole: new Int8Array(16),
  visibility: new Uint8Array(16),
};
state.sim.unlockedZoneTier = 1;
state.sim.winMode = WIN_MODE.SUPREMACY;

state.world.zoneRole[5] = ZONE_ROLE.CORE;
state.world.zoneRole[6] = ZONE_ROLE.CORE;

const simOut = {
  playerAliveCount: 2,
  playerEnergyNet: 1,
  playerEnergyIn: 4,
  cpuAliveCount: 1,
  cpuEnergyIn: 1,
};
applyWinConditions(state, simOut, 31);

assert(simOut.gameResult === GAME_RESULT.LOSS, "core collapse must resolve to loss");
assert(simOut.winMode === WIN_MODE.CORE_COLLAPSE, "core collapse winMode drift");
assert(simOut.running === false, "core collapse must stop the run");
assert(simOut.runPhase === RUN_PHASE.RESULT, "core collapse must set result phase");

console.log("CORE_COLLAPSE_LOSS_OK canonical core extinction verified");
