...
assert(state.sim.patternCatalog && typeof state.sim.patternCatalog === "object", "patternCatalog must exist on bootstrap");
assert(state.sim.patternBonuses && typeof state.sim.patternBonuses === "object", "patternBonuses must exist on bootstrap");
assert(Object.keys(state.sim.patternCatalog).length === 0, "patternCatalog must start empty");
assert(Object.keys(state.sim.patternBonuses).length === 0, "patternBonuses must start empty");
...
assert(ArrayBuffer.isView(state.world.zoneRole), "zoneRole must be a typed array");
assert(ArrayBuffer.isView(state.world.zoneId), "zoneId must be a typed array");
...
assert(state.world.zoneMeta && typeof state.world.zoneMeta === "object" && !Array.isArray(state.world.zoneMeta), "zoneMeta must be an object");
