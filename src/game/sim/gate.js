// ============================================================
// Simulation Gate — Specialized patch validation (Core)
// ============================================================
//
// Purpose:
// - Kernel mutationMatrix can stay broad (bandwidth + high-level gating).
// - Simulation requires stricter, domain-specific rules to prevent drift:
//   - no unknown /world or /sim keys
//   - typed array constructor + length invariants
//   - patch count limits (DoS hardening)
//
// Source of truth: project.manifest.js exports `simGate`.
...
function expectedLen(expr, N, TRAIT_COUNT) {
  if (expr === "N") return N;
  if (expr === "N*TRAIT_COUNT") return N * TRAIT_COUNT;
  return null;
}
...
export function assertSimPatchesAllowed(manifest, state, actionType, patches) {
  ...
  for (const p of patches) {
    ...
    // Only validate /world/* and /sim/* strictly (simulation-specific); meta is kernel-gated.
